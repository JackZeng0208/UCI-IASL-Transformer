...
context = zmq.Context()
tensor_socket = context.socket(zmq.REQ)  # REQ (request) socket for sending tensor requests
tensor_socket.connect(f"tcp://192.168.0.132:1919")  # Connect to the server's port
...
def send_tensor_to_server_zmq(draft_tokens_list):
    """
    Send draft tokens to the server via ZeroMQ.
    """
    data = {'draft_tokens': draft_tokens_list}
    tensor_socket.send_string(json.dumps(data))  # Send JSON data
    response = tensor_socket.recv_string()  # Receive acknowledgment
    print('send from edge to server', response)

def get_tensor_from_server_zmq():
    """
    Receive target model's logits from the server via ZeroMQ.
    """
    tensor_socket.send_string('get_tensor')  # Send request for tensors
    response = tensor_socket.recv_string()  # Receive JSON response
    target_prob_hist = json.loads(response)['target_prob_hist']
    return target_prob_hist
...
def edge_speculative_sampling(prefix : torch.Tensor, 
    ...
    while prefix.shape[1] < T: 
        ...
        send_tensor_to_server_zmq(draft_tokens_list=draft_tokens_list)
        
        #/get_tensor_from_server
        #get the logits
        target_model_mesg_dict = target_model_history = get_tensor_from_server_zmq()
